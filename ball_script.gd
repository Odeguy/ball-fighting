...
func _ready() -> void:
	...
	$RigidBody2D.linear_velocity = Vector2(get_viewport_rect().size.x / 2 - self.position.x  * -1 * lin_speed / 2, get_viewport_rect().size.y / 2 - self.position.y  * -1 * lin_speed / 2)
...
func _physics_process(delta: float) -> void:
	...
	speed_bonus = get_velocity_mag() / 500 + $RigidBody2D.angular_velocity / 8
